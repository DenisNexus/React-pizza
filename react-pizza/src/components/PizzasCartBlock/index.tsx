...
import { CartItem, addToCart,minusItem,removeToCart } from '../../redux/slices/cartSlice'
import { useAppDispatch } from '../../redux/store'


const  PizzasCartBlock:React.FC<CartItem> =  ({name,price,imageUrl,sizes,types,count,id,}) =>  {
  const dispatch = useAppDispatch()
  const  onClickPlus = () =>{
    dispatch(addToCart({id}as CartItem))
  }
  const onClickMinus=()=>{
    dispatch(minusItem({id}as CartItem))
  }
  const onClickDelete =()=>{
    dispatch(removeToCart({id}as CartItem))
  }
    return (
